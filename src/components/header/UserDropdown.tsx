...
export default function UserDropdown() {
  ...
  if (isLoading) {
    return (
      <div className="flex items-center">
        <div className="h-11 w-11 rounded-full bg-gray-200 dark:bg-gray-700 animate-pulse"></div>
        <div className="ml-3">
  ...
  // Don't render dropdown if no user is signed in
  if (!session?.user) {
    return null;
  }
    ...
    <div className="relative">
      ...
      >
        <div className="px-3 py-2">
          ...
          {user.role && (
            <span className="mt-1 inline-block px-2 py-1 text-xs font-medium bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-200 rounded-full">
              {user.role}
            </span>
          )}
        </div>
        ...
        <button
          ...
          className="flex items-center gap-3 px-3 py-2 mt-3 font-medium text-gray-700 rounded-lg group text-theme-sm hover:bg-red-50 hover:text-red-700 dark:text-gray-300 dark:hover:bg-red-900/20 dark:hover:text-red-300 transition-colors duration-200"
        >
          <svg
            className="fill-gray-500 group-hover:fill-red-700 dark:fill-gray-400 dark:group-hover:fill-red-300"
            width="24"
            height="24"
            viewBox="0 0 24 24"
            fill="none"
