...
export default function MeetingMinutesList() {
  ...
  const filteredMinutes = minutes.filter((minute) => filter === 'all' || minute.status === filter);
            ...
            {filteredMinutes.map((minute) => (
              <tr key={minute.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <span
                    className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${statusColors[minute.status]}`}
                  >
