...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (ministry_id) {
      ...
      await query('UPDATE ministries SET cabinet_secretary = NULL WHERE cabinet_secretary = $1', [
        params.id,
      ]);
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const userCheck = await query('SELECT id, name FROM users WHERE id = $1', [params.id]);
